"""
Piste B — 2024 out-of-sample validation run.

Same variant matrix as run_piste_b (2026-H1 in-sample).  Running on 2024
full-year data tests whether the SL=20/30 + mtf_smc() TP improvement holds
out-of-sample, satisfying the CLAUDE.md "multiple market periods" requirement.

Variants
--------
  REF       V2A_L3 reference — SL=6, scalp() TP (1.5R/2.5R/4R)
  PB_SL     SL=20/30 + mtf_smc() TP (1R/3R/5R/10R), no new gates
  PB_PAT    PB_SL + candle pattern gate (hammer/engulfing)
  PB_DB     PB_PAT + require_daily_bias=True
  PB_OTE    PB_DB  + require_ote=True  (full V3 filter set)

Usage
-----
    python -m zeus.backtest.run_piste_b_2024
"""
...
# ── Variant definitions (identical to run_piste_b) ────────────────────────────
VARIANTS: list[dict] = [
    {
...
def main() -> None:
    ...
    print(f"  1M bars (2024): {len(df_ltf):,}")
    ...
    print()
    _sep("═")
    flags_hdr = "SL  TP     PAT DB  OTE"
    print(f"  {'ID':8}  {flags_hdr:22}  {'Variant':<26}  {'Trades':>6}  {'WR%':>5}  "
    ...
    out = {
        "meta": {
            ...
            "note": "Out-of-sample 2024 validation of Piste B variants",
            "reference": "V2A_L3 (score=3, SL=6, approach+sweep lb=3, scalp() TP)",
            "piste_b": "1M candle pattern (hammer/engulfing) + mtf_smc() TP (1R/3R/5R/10R)",
        },
        "variants": [
    ...
    out_path = RESULTS_DIR / "piste_b_xauusd_2024.json"
